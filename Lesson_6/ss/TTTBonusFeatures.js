...
function computerBestMove (board) {
  ...
  let potentialBoard = Object.assign({},board);
  let trialSquares = emptySquares(board).slice();
  let bestMove = trialSquares[0];

  for (let idx = 0; idx < trialSquares.length; idx += 1) {
    potentialBoard[trialSquares[idx]] = MARKER['computer'];
    winner = miniMax (potentialBoard, 'computer');
    if (winner === 'computer') {
      bestMove = trialSquares[idx];
      break;
    } else if (winner === 'tie') {
      bestMove = trialSquares[idx];
    }
    potentialBoard[trialSquares[idx]] = MARKER['initial'];
  }
  return bestMove;
}
...
function miniMax (board, player) {
  ...
  if (player === 'computer') {
    for (let idx = 0; idx < trialSquares.length; idx += 1) {
      potentialBoard[trialSquares[idx]] = MARKER['computer'];
      if (detectWinner(potentialBoard) === 'computer') {
        winner = 'computer';
        break;
      } else {
        winner = miniMax (potentialBoard, 'human');
        if (winner === 'computer') break;
      }
      trialSquares[idx] = MARKER['initial'];
    }
  } else if (player === 'human') {
    for (let idx = 0; idx < trialSquares.length; idx += 1) {
      potentialBoard[trialSquares[idx]] = MARKER['human'];
      if (detectWinner(potentialBoard) === 'human') {
        winner = 'human';
        break;
      } else {
        winner = miniMax (potentialBoard, 'computer');
        if (winner === 'human') break;
      }
    }
  }
  return winner;
